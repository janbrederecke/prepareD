...
#' @param .include If .include = TRUE, variables recoded as 0/1 within the
#' function are returned in the list-output - requires .output = "list".
...
find_binary <- function(.data
  ...
  if (.output == "list") {
    ...
    unique_values <-
      lapply(.data[, sort(c(bin_var, yn_var, suspected_var))],
        function(x) {
